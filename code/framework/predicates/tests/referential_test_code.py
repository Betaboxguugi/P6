...
import sqlite3
from framework.predicates import ReferentialPredicate
...
time_dim = DimRepresentation('timeDim', 'timeid', ['day', 'month', 'year'], dw_conn, query=query2)
location_dim = DimRepresentation('locationDim', 'locationid', ['city', 'region'], dw_conn, ['city'], query=query3)
facttable = FTRepresentation('factTable', ['bookid', 'locationid', 'timeid'], ['sale'], dw_conn, query=query4)
dw = DWRepresentation([book_dim, time_dim, location_dim], [facttable], dw_conn)

ref_tester = ReferentialPredicate()
ref_tester.run(dw)
report = ref_tester.report()  # Not testing using the framework, so it looks ugly this way
report.run()
